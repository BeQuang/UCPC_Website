...
import { saveTemplateMail } from '../../../services/adminService/mailService';
...
function EmailBuilder() {
  ...
  const HandleSave = async () => {
    ...
    let res = await saveTemplateMail(templateName, htmlTemplate)
    if (res && res.EC === 0) {
      alert('Save success')
    } else {
      alert('Save fail')
  ...
  return (
    <StyledApp>
      <StyledHead>
        ...
        <span style={{ marginLeft: '20px' }}>Template Name: {templateName}</span>
      </StyledHead>
      <StyledBuilderWrap>
        <ErrorBoundary>
